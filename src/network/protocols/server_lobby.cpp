...
void ServerLobby::computeNewRankings()
{
    ...
    {
        is_eliminated.push_back(w->getKart(i)->isEliminated());
        uint32_t id = RaceManager::get()->getKartInfo(i).getOnlineId();
        ids.push_back(id);
    ...
    // Update the server_lobby.cpp values from what we calculated
    for (unsigned i = 0; i < player_count; i++)
    {
        uint32_t id = ids[i];
        m_raw_scores.at(id) = new_state.raw_scores[i];
        m_scores.at(id) = new_state.shown_scores[i];
        m_rating_deviations.at(id) = new_state.deviations[i];
        m_num_ranked_disconnects.at(id) = new_state.masks[i];
        m_max_scores.at(id) = new_state.max_scores[i];
        m_num_ranked_races.at(id) = new_state.num_games[i];
    }
    ...
    for (unsigned i = 0; i < player_count; i++)
    {
        ...
        double change = m_scores.at(id) - old_state.shown_scores[i];
