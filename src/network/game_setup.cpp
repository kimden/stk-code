        ...
        RaceManager::get()->isBattleMode())
    {
        ...
        if (RaceManager::get()->isSoccerMode())
        {
            if (isSoccerGoalTarget())
                ...
                RaceManager::get()->setTimeTarget((float)m_laps * 60.0f + m_additional_seconds);
        }
        else
        {
            RaceManager::get()->setHitCaptureTime(m_hit_capture_limit,
                m_battle_time_limit + m_additional_seconds);
        }
...
// ----------------------------------------------------------------------------
void GameSetup::setRace(const PeerVote &vote, float additional_seconds)
{
    m_tracks.push_back(vote.m_track_name);
    m_laps = vote.m_num_laps;
    m_reverse = vote.m_reverse;
    m_additional_seconds = additional_seconds;
}   // setRace
// ----------------------------------------------------------------------------
