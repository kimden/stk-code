...
void ItemPolicy::fromString(std::string& input) {
    ...
    // 1 section starting on lap 1 with no rules, all data to 0, deg and fuel consumption to 1x, and a length-0 item vector
    ...
    for (int i = 0; i < section_number; i++) {
        ...
        StringUtils::fromString(fetch(params, cumlen), tmp.m_fuel_mult);
        cumlen++;
